// from https://github.com/kennetpostigo/lwt-node/blob/master/src/path.re/
...
let format = pathObject =>
  switch (pathObject) {
  | {dir: Some(dir), base: Some(base), _} => dir ++ sep ++ base
...
let normalize = path =>
  Util.handleDotDot(
    ~path=
      Util.removeDuplicateForwardSlash(
        ~path=Util.resolveRelativePaths(~path, ~newPath=""),
        ~newPath="",
      ),
    ~newPath="",
  );
...
let resolve = paths => {
  ...
  Util.iterPathUntil(~condition=keepJoining, ~list=paths, ~f=onPath);
...
let relative = (~from, ~_to) => {
  ...
  let dirFrom =
    Filename.is_relative(pathFrom) ? resolve([pathFrom]) : pathFrom;
  let dirTo = Filename.is_relative(pathTo) ? resolve([pathTo]) : pathTo;
  if (dirFrom == dirTo) {
    "";
  } else {
    let (fromDiff, toDiff) =
      Util.removeCommonPath(~path1=dirFrom, ~path2=dirTo);
    Util.getRelativePath(~path1=fromDiff, ~path2=toDiff, ~isFilePath=false);
  };
};
...
let relativeFilePath = (~from, ~_to) => {
  ...
  if (dirFrom == dirTo) {
    ...
    let (fromDiff, toDiff) =
      Util.removeCommonPath(~path1=dirFrom, ~path2=dirTo);
    Util.getRelativePath(~path1=fromDiff, ~path2=toDiff, ~isFilePath=true);
  };
};
            ...
            pathLength == 1
              ...
              : Some(
                  String.sub(
                    p,
                    0,
                    String.rindex_from(p, pathLength - 1, charSep),
                  ),
                );
          base :=
            Some(
