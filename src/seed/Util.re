...
let rec removeCommonPath = (~path1, ~path2) => {
  ...
  let subp2 =
    switch (String.index_from(path2, 1, charSep)) {
...
let rec getRelativePath = (~path1, ~path2, ~isFilePath): string => {
  ...
  | exception Not_found when String.length(path1) > 0 && isFilePath == false =>
    ".." ++ sep ++ path2
  | exception _ => path2
  };
};
    ...
    | _ =>
      ...
      | "../" =>
        handleDotDot(
          ~path=updatePath,
          ~newPath=
            String.sub(newPath, 0, String.length(newPath) - 1) ++ chunk,
        )
